...
We can extend this vector space concept to deal with entire texts by averaging all of the word vectors for each text into text-vector representations of each text and then calculating the similarity of the text-vectors using same cosign similarity measure.  The more similar the cosign similarities of the two text-vectors, the more semantically similar the two texts are.

This process could be extended yet again to classify and compare entire corpora.
...
-------------------------------------
RAGs are a popular way of working directly with LLM models to leverage their power to direct people towards libraries of existing texts that relate to their specific prompts/queries.  For example, Help Desk chatbots frequently use RAGS to deliver specific help documentation to users based on their prompts rather than simply generating a response directly from the LLM. Using a RAG framework, for example, one could create a chatbot that receives medical queries and uses RAG to find relevant, peer-reviewed journal papers and returns a relevant list of those articles to the user instead of an AI generated summary of everything ever said about the query.

Here we will build a simple RAG application based on Ollama text embedding vectors.  For this example, we are providing the retrieval library and user prompt programmatically, but the code could be easily modified to use a training corpus that is stored on disk and to receive the prompt programmatically (as we did in our chatbot example earlier).  
...
We'll also here provide our hard-coded user prompt.  As noted earlier, in a real-world application this would be dynamically received, as in our earlier Chatbot exercise.
...
Our *prompt_vector* variable now contains the vector representation of our prompt. With this in hand, we can now compare the prompt_vector to each of the training texts vectors using a cosign similarity measure.  The most similar vector represents the text from our document set that most closely semantically matches our prompt.
